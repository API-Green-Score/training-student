...
### 1. Introduction et mise en place des outils (20 min)
...
Exercice 1 : Création d’un système de logs pour suivre les appels API (25 min)
...
Développer un service de log en Java Spring Boot qui :
...
(Les étudiants doivent ensuite modifier et améliorer ce code)
...
url, response.getStatusCode(), payloadSize, duration);
...
Exercice 2 : Choix du format d’échange de données et impact environnemental (20 min)
...
Effectuer une requête sur OpenLibrary en JSON et en XML via Postman.

https://openlibrary.org/search.json?q=harry+potter

Convertir au format XML https://www.site24x7.com/fr/tools/xml-en-json.html

Vérifier la taille des payloads avec Postman ou tout autre outil de votre choix et les enregistrer dans le système de logs.

Comparer les tailles des fichiers JSON et XML et analyser les logs générés.

Déduire l’impact en termes de bande passante et stockage.

faire cette comparaison avec un volume d’appel de 1 millions d’appels par jour à l’API
...
Exercice 3 : Réduction des données transférées avec Java Spring Boot (25 min)
...
Créer un service API en Java Spring Boot qui renvoie une liste d’adresses.

Implémenter la pagination et les filtres pour limiter les données renvoyées.

Vérifier avec les logs que les optimisations réduisent bien le volume de données.
...
Exercice 4 : Utilisation du cache pour limiter les requêtes inutiles (20 min) [Facultatif]
...
Modifier le service en Java Spring Boot pour cacher les réponses des API externes.

Comparer les temps d’exécution avec et sans cache via les logs.
...
Recherche d'API sur data.gouv.fr
L' API Adresse permet d'interroger facilement la Base…
API Spotify
...
Exemple : Une réponse API de 500 ko compressée peut descendre à 100 ko → gain de bande passante et d’énergie.
